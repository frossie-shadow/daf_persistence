...
class PosixStorage(StorageInterface):
    ...
    def write(self, butlerLocation, obj):
        ...
        with SafeFilename(os.path.join(self.root, butlerLocation.getLocations()[0])) as locationString:
            logLoc = LogicalLocation(locationString, additionalData)
            # Create a list of Storages for the item.
            storageList = StorageList()
            storage = self.persistence.getPersistStorage(butlerLocation.getStorageName(), logLoc)
    ...
    def read(self, butlerLocation):
        ...
        for locationString in butlerLocation.getLocations():
            ...
            storageList = StorageList()
            storage = self.persistence.getRetrieveStorage(butlerLocation.getStorageName(), logLoc)
            storageList.append(storage)
            finalItem = self.persistence.unsafeRetrieve(butlerLocation.getCppType(), storageList,
                                                        additionalData)
